...
export default function DiferenciaVector() {
  ...
  return (
    <SectionReveal id="diferencia" className="scroll-mt-24 bg-surface-container-lowest/50 py-24 md:py-32">
      <div className="mx-auto max-w-container-max px-margin-mobile md:px-margin-desktop">
        ...
        >
          ...
          <h3 className="font-display-lg text-2xl leading-tight text-on-surface md:text-4xl">{porQueIntro.titulo}</h3>
        ...
        <div className="grid grid-cols-1 gap-gutter md:grid-cols-3">
          {porQuePilares.map(({ id, Icon, titulo, texto, IconSecondary }, i) => (
            <motion.article
              key={id}
              className="border border-outline bg-surface p-8 text-left md:p-10"
              initial={reduce ? false : { opacity: 0, y: 16 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              transition={{ delay: 0.08 * i, duration: 0.45, ease: [0.22, 1, 0.36, 1] }}
              whileHover={reduce ? {} : { y: -4, borderColor: "rgba(173, 206, 186, 0.35)" }}
            >
              ...
              <h4 className="font-headline-lg mb-3 text-xl text-on-surface">{titulo}</h4>
